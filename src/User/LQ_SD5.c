    ...
    while(1)
    {
        switch(KEY_Read(1))     
        {
          ...
          case 3: 
            LED_Reverse(3); 
            duty -= 10;
            if(duty < 400)        //根据自己的实际情况对舵机打角进行限制，防止卡死
            {
                duty = 400;
            }
			FTM_PwmDuty(FTM3, FTM_CH6, duty); 
...
void Z_Test_FTMSD5(void)
{
    ...
    while(1)
    {
            ...
            LED_Reverse(2); 
            duty_z -= 10;
            if(duty_z < 600)        //根据自己的实际情况对舵机打角进行限制，防止卡死
            {
                duty_z = 600;break;
            }
            FTM_PwmDuty(FTM3, FTM_CH6, duty_z); 
			FTM_PwmDuty(FTM3, FTM_CH7, duty_z); 
...
void M_Test_FTMSD5(void)
{
    ...
    uint16_t duty_y = 760;
            ...
            FTM_PwmDuty(FTM3, FTM_CH6, duty_y); 
			FTM_PwmDuty(FTM3, FTM_CH7, duty_y);
...
void Y_Test_FTMSD5(void)
{
    ...
    while(1)
    {
            ...
            LED_Reverse(2); 
            duty_y += 10;
            if(duty_y > 1000)        //根据自己的实际情况对舵机打角进行限制，防止卡死
            {
                duty_y = 1000;break;
            }
            FTM_PwmDuty(FTM3, FTM_CH6, duty_y); 
...
void mduoji_FTMSD5(double x)
{
    ...
    uint16_t duty = 760+x*2;
  
            if(duty< 700)        //根据自己的实际情况对舵机打角进行限制，防止卡死
            {
                duty = 700;
            }
            else if(duty > 800)        //根据自己的实际情况对舵机打角进行限制，防止卡死
            {
                duty = 800;
            }
            FTM_PwmDuty(FTM3, FTM_CH6, duty); 
...
void m1duoji_FTMSD5(double x)
{
    ...
    uint16_t duty = 760+x*3;
  
            if(duty< 620)        //根据自己的实际情况对舵机打角进行限制，防止卡死
            {
                duty = 620;
            }
            else if(duty > 920)        //根据自己的实际情况对舵机打角进行限制，防止卡死
            {
                duty = 920;
            }
            FTM_PwmDuty(FTM3, FTM_CH6, duty); 
...
void duoji_FTMSD5(double x)
{
    ...
    uint16_t duty = 760+x*7;
  
            if(duty< 400)        //根据自己的实际情况对舵机打角进行限制，防止卡死
            {
                duty = 400;
            }
            else if(duty > 1200)        //根据自己的实际情况对舵机打角进行限制，防止卡死
            {
                duty = 1200;
            }
            FTM_PwmDuty(FTM3, FTM_CH6, duty); 
